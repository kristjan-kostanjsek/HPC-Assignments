...
void initUV(float *U, float *V, int n, int local_rows, int rank, int size) {
    ...
    int r = n / 8;
    int global_row_offset = rank * (n / size);
...
void gray_scott(float* U, float* V, float* U_new, float* V_new,
                int n, int local_rows, int steps, float dt, float du, float dv, float f, float k,
                int rank, int size, MPI_Comm comm) {

    float *recvbuf_up_U = (float*) malloc(n * sizeof(float));
    float *recvbuf_down_U = (float*) malloc(n * sizeof(float));
    float *recvbuf_up_V = (float*) malloc(n * sizeof(float));
    float *recvbuf_down_V = (float*) malloc(n * sizeof(float));
    ...
    for (int step = 0; step < steps; step++) {
        // Exchange U halos
        MPI_Sendrecv(U + n, n, MPI_FLOAT, up_rank, 0,
                    recvbuf_down_U, n, MPI_FLOAT, down_rank, 0, comm, MPI_STATUS_IGNORE);
        MPI_Sendrecv(U + (local_rows-2) * n, n, MPI_FLOAT, down_rank, 1,
                    recvbuf_up_U, n, MPI_FLOAT, up_rank, 1, comm, MPI_STATUS_IGNORE);

        // Exchange V halos
        MPI_Sendrecv(V + n, n, MPI_FLOAT, up_rank, 2,
                    recvbuf_down_V, n, MPI_FLOAT, down_rank, 2, comm, MPI_STATUS_IGNORE);
        MPI_Sendrecv(V + (local_rows-2) * n, n, MPI_FLOAT, down_rank, 3,
                    recvbuf_up_V, n, MPI_FLOAT, up_rank, 3, comm, MPI_STATUS_IGNORE);

        // Copy halo rows into U, V
        for (int j = 0; j < n; j++) {
            U[0 * n + j] = recvbuf_up_U[j];
            U[(local_rows-1)*n + j] = recvbuf_down_U[j];

            V[0 * n + j] = recvbuf_up_V[j];
            V[(local_rows-1)*n + j] = recvbuf_down_V[j];
        }
...
int main(int argc, char *argv[]) {
    ...
    int local_rows = n/size + 2; // +2 for ghost rows
    ...
    double t0 = MPI_Wtime();
    gray_scott(U, V, U_new, V_new, n, local_rows, steps, dt, du, dv, f, k, rank, size, MPI_COMM_WORLD);
    double t1 = MPI_Wtime();
    ...
    MPI_Gather(V + n, (local_rows-2)*n, MPI_FLOAT, V_global, (local_rows-2)*n, MPI_FLOAT, 0, MPI_COMM_WORLD);

    // optionally visualize the end result (V grid)
    if (rank == 0) {
        save_grayscale_image("V_end_mpi.png", V_global, n);
        free(V_global);
    }

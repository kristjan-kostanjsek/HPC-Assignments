// gcc -o parallel_basic_seam parallel_basic_seam.c -lm -fopenmp -O2
...
#define REPETITIONS 32 // TODO change number of repetitions here
...
void remove_seam_copy(unsigned char *image_in, unsigned char *image_out, int width, int height, int cpp, int *seam) {
    ...
    #pragma omp parallel for
    for (int y = 0; y < height; y++) {
        int in_index = y * width * cpp;  // Set the start index for the current row
        int out_index = y * (width - 1) * cpp;  // Start output index, skipping one column
...
int main(int argc, char *argv[]) {
    ...
    int *seam = (int *)malloc(num_threads * height * sizeof(int));

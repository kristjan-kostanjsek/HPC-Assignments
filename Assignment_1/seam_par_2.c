...
void imp_cummulative_energy(int width, int height, float **energy_map, int num_threads) {
    const int T_H = 8; //width / num_threads; // Triangle height (adjust as needed)
...
int main(int argc, char *argv[]) {
    ...
    const int REPETITIONS = num_pixels_to_remove / 1; // Number of seams removed from the image
    ...
    for (int rep = 0; rep < REPETITIONS; rep++) {
        ...
        bool* seam_mask = imp_find_cheapest_path(width - rep, height, energy_map, 1);

        // 3. REMOVE SEAM FROM IMAGE (Copy the image to new image, without the seam)
        imp_remove_seam_copy(image_in, image_out, width - rep, height, cpp, seam_mask, 1);
